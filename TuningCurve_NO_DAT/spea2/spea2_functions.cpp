...
void environmentalSelection()
{
  ...
  if(verbose) {
    char tmp [1024];
    sprintf(tmp, " spea2: #%d after truncation 1st front size (%d", nc, fitness_bucket[0]);
    if(fitness_bucket[0] > alpha) sprintf(tmp, "%s*)\n", tmp);
    else sprintf(tmp, "%s)\n", tmp);

    msg_to_file(diag_file, tmp);

    /* print indices of non-dominated individuals                 */
    /* - non-dominated individuals have fitness = 0; total number */
    /*   of non-dominated individuals are in fitness_bucket[0]    */
    /*   (array index of fitness_bucket is fitness value)         */
    /* - search through population for zero fitness individuals   */
    /*   and print their ID numbers (index values in ind_array),  */
    /*   objective, and constraints values (index is worthless).  */
    if(fitness_bucket[0] > 0) {
      char tmp2 [5000];
      sprintf(tmp2, " spea2: #%d after truncation 1st front ids, objectives (%d), and constraints (%d):\n",nc,dim,con);
      msg_to_file(diag_file, tmp2);
      for(int i = 0; i < pp_all->size; i++) {
        if(pp_all->ind_array[i]->fitness == 0) {
          sprintf(tmp2, "%d", pp_all->ind_array[i]->index);
          for(int j = 0; j < dim ; j++) {
            sprintf(tmp2, "%s %g", tmp2,pp_all->ind_array[i]->f[j]);
          }
          for(int j = 0; j < con ; j++) {
            sprintf(tmp2, "%s %g", tmp2,pp_all->ind_array[i]->c[j]);
          }
          sprintf(tmp2, "%s\n", tmp2);
          msg_to_file(diag_file, tmp2);
        }
